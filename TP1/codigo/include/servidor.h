...
int obtener_telemetria(int socket, socklen_t size, struct sockaddr_un prueba)
{
    ...
    //recvfrom ( socket, buffer, sizeof(buffer), 0, (struct sockaddr *)&prueba, &size );
    //if (strcmp(buffer, "ACK") == 0)
    //{
    //    printf("\nServidor recibio ACK.\n");
    //}
    ...
    control_error = recvfrom ( socket, id, sizeof(id), 0, (struct sockaddr *)&prueba, &size );
    if (control_error < 0)
    {
    ...
    control_error =recvfrom ( socket, uptime, sizeof(uptime), 0, (struct sockaddr *)&prueba, &size );
    if (control_error < 0)
    {
        perror("lectura de socket");
        exit(1);
    ...
    control_error = recvfrom ( socket, version, sizeof(version), 0, (struct sockaddr *)&prueba, &size );
    if (control_error < 0)
    {
        perror("lectura de socket");
        exit(1);
    ...
    control_error = recvfrom ( socket, consumo, sizeof(consumo), 0, (struct sockaddr *)&prueba, &size );
    if (control_error < 0)
    {
        perror("lectura de socket");
        exit(1);
